...
def file_counter(category_sets: set) -> None :
    ...
    with open("target.txt", "r", encoding='utf-8') as f: 
        ...
        for readline in readlines :
            ...
            # 확인용 텍스트 파일 만든다.
            with open("counter_with_target.txt", "a", encoding='utf-8') as tc:
                tc.write(f"{nomralized_src}\n")
            # 숫자만 나오게해서 복사 붙여넣기를 편하게한다
            with open("counter_without_target.txt","a", encoding='utf-8') as c :
                c.write(f"{str(counter[design_number])}\n")
                

if __name__=="__main__" :
    category_sets = make_folder_structure(text_file_name="target.txt") # 폴더구조 생성
    parse_file(category_sets=category_sets, root_path="미분류_검수완료_aug", save_in="크롤링") # root_path는 폴더구조를 재정의하고자하는 src
